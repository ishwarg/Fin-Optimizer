    ...
    class Rocket
    {
       ...
       public double Ns, propmass;
        ...
        public double TotalCP(Fin Lower, Fin Upper, double Mach)
        {
           ...
           CP= (NoseCn * NoseCp + Fin.finCP(Lower, Mach)*Fin.CN(Lower,Mach) + Fin.finCP(Upper, Mach) * Fin.CN(Upper,Mach)) / (Fin.CN(Lower,Mach)+ Fin.CN(Upper,Mach)+NoseCn);
       ...
       public double drag(Conditions conditions, int stage)
        {
            ...
            {
                cd = Cd1(conditions);
                force = cd * 0.5 * V * V * conditions.rho * Aref;
            }
            else
            {
                cd = Cd2(conditions);
        ...
        public double netforce(double time,Conditions conditions, int stage, double firststagetime)
        {
            double force;
            ...
            //if (time >= 16.22 && time < 16.23)
            //{
            //    Console.WriteLine(Functions.ThrustInterpolator(time - firststagetime, Simulator.UpperThrust));
            //}
            return force;
        }
        public void avx(double time, Conditions conditions, int stage, double firststagetime)
        {
            double net = netforce(time, conditions, stage,firststagetime);
